       ...
       <form class="form-inline" method="GET">
        <div class="form-group" style="margin:10px 10px 0px 0px">
          <div class="input-group">
            <div class="input-group-addon">類別名稱</div>
     ...
     <div class="row">
         ...
         <div class="col-sm-6">
             <div class="m-b-30 text-right">
                 <a href="<?=_Web_Url?>/catalog/catalog_admin/add" id="addToTable" class="btn btn-success waves-effect waves-light">新增 <i class="mdi mdi-plus-circle-outline"></i></a>
             </div>
         </div>
     </div>
     <div class="row">
           ...
           <tbody>
             ...
             <tr>
                 <td class="text-center tg-checkbox">
                    <input id="checkbox<?=$x?>" type="checkbox" name="number[]" value="<?=$row -> Catalog_Id?>">
                 </td>
                 <td><?=($row ->Catalog_Status)?'啟用':'關閉'?></td>
                 <td><?=$row -> Catalog_Sort?></td>
                 <td><?=$CatalogList[$row -> Catalog_ParentId]?></td>
                 <td><?=$row -> Catalog_Name?></td>
                 <td class="actions" >
                   <a href="<?=_Web_Url?>/catalog/catalog_admin/edit/<?=$row -> Catalog_Id?>/<?=$Page."?".http_build_query($this->input->get())?>" class="on-default edit-row" data-toggle="tooltip" data-placement="top" title="" data-original-title="修改">
				...
				$.ajax({
					...
					success: function(data){
						if(data.Status != '0000')
						{
							...
							alert('刪除成功！');
							window.location = "<?=_Web_Url?>/catalog/catalog_admin";
							return false;
						}
					}
				});
