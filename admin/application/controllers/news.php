<?
	Class news extends CI_Controller{
		...
		//=================================== 網站頁面處理 =========================================
		function news_admin($Type = 'list',$Id = 0,$Page = 0)
		{
			$this -> load -> model('news_process');
			$ModelName	= 'news';
			...
			switch($Type)
			{
				case 'add':
					$Data['Page']	= $Page;
					$Data['Type'] = $Type;
					$Data['Right_Contents']	= $this -> load -> view('news/news_form',$Data,true);
				case 'edit':
					$DataArray	= $this -> news_process -> get_DataById($Id);
					$Data['Page']	= $Page;
					$Data['Type'] = $Type;
					...
					break;
				default:
					$DataArray = $this->news_process->get_DataList($Page);
					$Data['Page']	= $Page;
					$Data['DataList'] = $DataArray['DataList'];
					$Data['PageList'] = $DataArray['PageList'];
					$Data['TotalNum'] = $DataArray['TotalNum'];
					$Data['Right_Contents']	= $this -> load -> view('news/news_list',$Data,true);
					break;
			}
		...
		function Add_Data()
		{
			$Status_Num	= $this ->news_process-> Add_Data($this->input->post());
			//刪除快取
		...
		function Edit_Data()
		{
			$Status_Num	= $this->news_process->Edit_Data($this->input->post());
			//刪除快取
			$this -> cache -> del_cache();
		...
		function Del_Data()
		{
			$Status_Num	= $this->news_process->Del_Data($this->input->post());
			//刪除快取
			$this -> cache -> del_cache();
