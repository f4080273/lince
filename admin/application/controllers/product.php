<?
	Class product extends CI_Controller{
		...
		//=================================== 網站頁面處理 =========================================
		function product_admin($Type = 'list',$Id = 0,$Page = 0)
		{
			$ModelName	= 'product';
			...
			{
				case 'add':
					$Data['get_CatalogList'] = $this-> product_process ->get_CatalogList();
					$Data['Page']	= $Page;
					$Data['Type'] = $Type;
					$Data['Right_Contents']	= $this -> load -> view('product/product_form',$Data,true);
					break;
				case 'edit':
					$Data['get_CatalogList'] = $this-> product_process ->get_CatalogList();
					$Data['Page']	= $Page;
					$Data['Type'] = $Type;
					$Data['DataArray'] = $this -> product_process -> get_DataById($Id);
					$Data['Right_Contents']	= $this -> load -> view('product/product_form',$Data,true);
					break;
				default:
					$Data['get_CatalogList'] = $this-> product_process ->get_CatalogList();
					$DataArray = $this->product_process->get_DataList($Page);
					$Data['Page']	= $Page;
					$Data['DataList'] = $DataArray['DataList'];
					$Data['PageList'] = $DataArray['PageList'];
					$Data['TotalNum'] = $DataArray['TotalNum'];
					$Data['Right_Contents']	= $this -> load -> view('product/product_list',$Data,true);
					break;
			}
		...
		function Add_Data()
		{
			$Status_Num	= $this -> product_process -> Add_Data($this->input->post());
			//刪除快取
		...
		function Edit_Data()
		{
			$Status_Num	= $this -> product_process -> Edit_Data($this->input->post());
			//刪除快取
			$this -> cache -> del_cache();
		...
		function Del_Data()
		{
			$Status_Num	= $this -> product_process -> Del_Data($this->input->post());
			//刪除快取
			$this -> cache -> del_cache();
